...
def get_encoder(prev_layer, num_of_layers, input_size, name):
    ...
    layers from the Transfer Learning model, ResNet50 here, and builds a
    ...
    resnet = get_resnet(num_of_layers, input_size)(prev_layer)
    reduce_1 = get_reduce_block(resnet, 128, True, name, 1)
...
def make_docs(name):
    ...
    path = os.path.join('docs', 'resnet')
    if not os.path.exists(path):
        os.mkdir(path)
    plot_model(model, to_file=os.path.join(path, 'autoencoder.png'),
               show_shapes=True, show_layer_names=True, rankdir='TB')
    plot_model(model.layers[1], to_file=os.path.join(path, 'resnet50.png'),
               show_shapes=True, show_layer_names=True, rankdir='TB')
