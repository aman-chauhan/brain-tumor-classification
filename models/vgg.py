...
def get_vgg(num_of_layers, input_size):
    '''
    Returns the non-trainable VGG19 layers as a Keras Model

    Given number of layers, this function iterates over the VGG19 model
    ...
    ----------
    num_of_layers: int
        number of layers you want to select from the VGG19 model
    input_size: int
    ...
    i = 0
    while(True):
        if vgg.layers[i].name == 'block{}_conv1'.format(num_of_layers):
            break
        i += 1
    model = Model(inputs=vgg.layers[0].input,
                  outputs=vgg.layers[i].output,
                  name='vgg')
...
def get_encoder(prev_layer, num_of_layers, input_size, name):
    ...
    layers from the Transfer Learning model, VGG19 here, and builds a
    ...
    vgg = get_vgg(num_of_layers, input_size)(prev_layer)
    reduce_1 = get_reduce_block(vgg, 128, True, name, 1)
    ...
    reduce_3 = get_reduce_block(dense_2, 128, True, name, 3)
...
def make_docs(name):
    ...
    path = os.path.join('docs', 'vgg')
    if not os.path.exists(path):
        os.mkdir(path)
    plot_model(model, to_file=os.path.join(path, 'autoencoder.png'),
               show_shapes=True, show_layer_names=True, rankdir='TB')
    plot_model(model.layers[1], to_file=os.path.join(path, 'vgg19.png'),
               show_shapes=True, show_layer_names=True, rankdir='TB')
