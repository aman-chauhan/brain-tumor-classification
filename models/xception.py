...
def get_xception(num_of_layers, input_size):
    '''
    Returns the non-trainable Xception layers as a Keras Model

    Given number of layers, this function iterates over the Xception
    ...
    ----------
    num_of_layers: int
        number of layers you want to select from the Xception model
    input_size: int
    ...
    i = 0
    while(True):
        if xception.layers[i].name == 'add_{}'.format(num_of_layers - 1):
            break
        i += 1
...
def get_encoder(prev_layer, num_of_layers, input_size, name):
    ...
    layers from the Transfer Learning model, Xception here, and builds
    ...
    xception = get_xception(num_of_layers, input_size)(prev_layer)
    reduce_1 = get_reduce_block(xception, 128, True, name, 1)
...
def make_docs(name):
    ...
    path = os.path.join('docs', 'xception')
    if not os.path.exists(path):
        os.mkdir(path)
    plot_model(model, to_file=os.path.join(path, 'autoencoder.png'),
               show_shapes=True, show_layer_names=True, rankdir='TB')
    plot_model(model.layers[1], to_file=os.path.join(path, 'xception.png'),
               show_shapes=True, show_layer_names=True, rankdir='TB')
